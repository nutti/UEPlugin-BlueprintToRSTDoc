...
void CreateRSTPropertyDoc(FString& Doc, RSTDocIndent& Indent, const FRSTDocProperty& Property)
{
	...
	// Property body.
	Doc += FString::Format(TEXT("{0}.. cpp:member:: {1} {2}\n\n"), {Indent.ToString(), Property.Type, Property.Name});

	Indent.Increment();

	// Property category.
	Doc += FString::Format(TEXT("{0}Category: {1}\n\n"), {Indent.ToString(), FString::Join(Property.Category, TEXT(", "))});
	...
	Doc += FString::Format(TEXT("{0}{1}\n\n"), {Indent.ToString(), Property.ToolTips});
...
void CreateRSTEdGraphPinDoc(FString& Doc, RSTDocIndent& Indent, const FRSTDocEdGraphPin& EdGraphPin)
{
	...
	{
		Doc += FString::Format(TEXT("{0}:{1} {2}: {3}\n"), {Indent.ToString(), ItemPrefix, EdGraphPin.Name, EdGraphPin.ToolTips});
	}
	else
	{
		Doc += FString::Format(TEXT("{0}:{1} {2}: {3} (Default: {4})\n"),
			{Indent.ToString(), ItemPrefix, EdGraphPin.Name, EdGraphPin.ToolTips, EdGraphPin.DefaultValue});
	}

	Doc += FString::Format(TEXT("{0}:{1} {2}: {3}\n"), {Indent.ToString(), TypePrefix, EdGraphPin.Name, EdGraphPin.Type});
}
...
void CreateRSTEdGraphDoc(FString& Doc, RSTDocIndent& Indent, const FRSTDocEdGraph& EdGraph)
{
	...
	for (auto& I : EdGraph.Inputs)
	{
		...
		{
			ArgDocs.Add(FString::Format(TEXT("{0} {1}"), {I.Type, I.Name}));
		}
		else
		{
			ArgDocs.Add(FString::Format(TEXT("{0} {1}={2}"), {I.Type, I.Name, I.DefaultValue}));
		}
	}
	...
	{
		ReturnDocs.Add(FString::Format(TEXT("{0}"), {O.Type}));
	}
	...
	Doc += FString::Format(TEXT("{0}Type: {1}\n\n"), {Indent.ToString(), EdGraph.ToolTips});

	// EdGraph category.
	Doc += FString::Format(TEXT("{0}Category: {1}\n\n"), {Indent.ToString(), FString::Join(EdGraph.Category, TEXT(", "))});

	// Edgraph access modifiers.
	Doc += FString::Format(
		TEXT("{0}Access Modifier: {1}\n\n"), {Indent.ToString(), FString::Join(EdGraph.AccessModifiers, TEXT(", "))});

	// Edgraph constant.
	Doc += FString::Format(TEXT("{0}Constant: {1}\n\n"), {Indent.ToString(), EdGraph.bIsConst ? TEXT("True") : TEXT("False")});

	// EdGraph flags.
	Doc += FString::Format(TEXT("{0}Flags: {1}\n\n"), {Indent.ToString(), FString::Join(EdGraph.Flags, TEXT(", "))});
...
void CreateRSTBlueprintDoc(FString& Doc, RSTDocIndent& Indent, const FRSTDocBlueprint& Blueprint)
{
	...
	// Content path.
	Doc += FString::Format(TEXT("Path: {0}/{1}\n\n"), {Blueprint.Path, Blueprint.Name});

	// Class name.
	if (Blueprint.ParentName.IsEmpty())
	{
		Doc += FString::Format(TEXT(".. cpp:class:: {0}\n\n"), {Blueprint.Name});
	}
	else
	{
		Doc += FString::Format(TEXT(".. cpp:class:: {0} : public {1}\n\n"), {Blueprint.Name, Blueprint.ParentName});
	}
	...
	{
		Doc += FString::Format(TEXT("{0}{1}\n\n"), {Indent.ToString(), Blueprint.ToolTips});
	}
...
void CreateRSTStructureDoc(FString& Doc, RSTDocIndent& Indent, const FRSTDocStructure& Structure)
{
	...
	{
		Doc += FString::Format(TEXT("{0}{1}\n\n"), {Indent.ToString(), Structure.ToolTips});
	}
...
void CreateRSTEnumerationFieldDoc(FString& Doc, RSTDocIndent& Indent, const FRSTDocEnumerationField& Field)
{
	...
	// Field body.
	Doc += FString::Format(TEXT("{0}.. cpp:enumerator:: {1} = {2}\n\n"), {Indent.ToString(), Field.Name, Field.Value});

	Indent.Increment();

	// Field description.
	Doc += FString::Format(TEXT("{0}{1}\n\n"), {Indent.ToString(), Field.ToolTips});
...
void CreateRSTEnumerationDoc(FString& Doc, RSTDocIndent& Indent, const FRSTDocEnumeration& Enumeration)
{
	...
	// Content path.
	Doc += FString::Format(TEXT("Path: {0}/{1}\n\n"), {Enumeration.Path, Enumeration.Name});

	// Enum name.
	Doc += FString::Format(TEXT(".. cpp:enum:: {0}\n\n"), {Enumeration.Name});
	...
	{
		Doc += FString::Format(TEXT("{0}{1}\n\n"), {Indent.ToString(), Enumeration.ToolTips});
	}
...
bool WriteRSTBlueprintDocs(const TArray<FRSTDocBlueprint>& Data, const FString& OutputDirectory, TArray<FString>& OutputDocsList)
{
	...
	if (Data.Num() >= 1)
	{
		...
		for (auto& D : Data)
		{
			...
			FString FilePath = FString::Format(TEXT("{0}{1}/{2}.rst"), {ClassDirectory, D.Path, D.Name});
...
bool WriteRSTStructureDocs(const TArray<FRSTDocStructure>& Data, const FString& OutputDirectory, TArray<FString>& OutputDocsList)
{
	...
	if (Data.Num() >= 1)
	{
		...
		for (auto& D : Data)
		{
			...
			FString FilePath = FString::Format(TEXT("{0}{1}/{2}.rst"), {StructDirectory, D.Path, D.Name});
	...
	const TArray<FRSTDocEnumeration>& Data, const FString& OutputDirectory, TArray<FString>& OutputDocsList)
{
	...
	if (Data.Num() >= 1)
	{
		...
		for (auto& D : Data)
		{
			...
			FString FilePath = FString::Format(TEXT("{0}{1}/{2}.rst"), {EnumDirectory, D.Path, D.Name});
