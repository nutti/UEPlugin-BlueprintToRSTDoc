...
void FBlueprintToRSTDocModule::StartupModule()
{
    ...
    Commands->MapAction(
        FBlueprintToRSTDocCommands::Get().Action,
        FExecuteAction::CreateRaw(this, &FBlueprintToRSTDocModule::CommandExecuted),
        FCanExecuteAction()
    );
    ...
    if (SettingsModule)
    {
        SettingsModule->RegisterSettings("Project", "Plugins", "BlueprintToRSTDoc",
            LOCTEXT("RuntimeSettingsName", "BlueprintToRSTDoc"),
            LOCTEXT("RuntimeSettingsDescription", "BlueprintToRSTDoc"),
            Settings);
    }
}
